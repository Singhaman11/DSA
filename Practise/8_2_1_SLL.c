...
void AddNode()
{
    ...
    for(int i = 0; i < n; i++)
    {
        ptr -> subject_start_ptr = (struct subject*) malloc(sizeof(struct subject));
        struct subject *temp2 = NULL;
        printf("Enter %d subject name", i);
        scanf("%s", ptr -> subject_start_ptr -> Subject);
        ptr -> subject_start_ptr -> next1 = NULL;
        if(ptr -> subject_start_ptr == NULL)
            ptr -> subject_start_ptr = ptr1;
        else
        {
            temp1 = subject_start_ptr;
...
{
    struct student *temp = start;
    struct subject *temp1 = subject_start_ptr;
    while(temp != NULL)
    {
        printf("Roll no. %d \n", temp -> RollNo);
